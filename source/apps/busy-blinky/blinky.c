...
static uint32_t computeFibonacciSequence(uint32_t fibCount)
{
   uint32_t aa = 0;
   uint32_t bb = 1;
...
static void toggleLedCPUHog(const void* arg)
{
   struct led_toggler* tog = (struct led_toggler*) arg;
   ...
   while (true)
   {
      ...
      result = computeFibonacciSequence(tog->period_ms * 16 * 1024);
